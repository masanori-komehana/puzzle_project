...
//    private static final int SUB_ACTIVITY = 100;
    ...
    private static final int RANKING_ACTIVITY = 102;
    private static final int STATISTICS_ACTIVITY = 103;
    private static final int REGISTER_ACTIVITY = 104;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode == RESULT_OK && requestCode == REGISTER_ACTIVITY){
            Toast.makeText(getApplicationContext(), dbHelper.getCurrentPlayer().getName()+"を選択しました。", Toast.LENGTH_LONG).show();
            updateCurrentPlayerName();
        }
        if(requestCode == PAZZLE_ACTIVITY){
            Pazzle pazzle = Pazzle.getInstance();
            ResultItem resultItem;
            if (pazzle.isActive()){
                dbHelper.addResult(1);
                resultItem = dbHelper.getResultLatest();
                Toast.makeText(getApplicationContext(), "パズルを停止しました。\n" +
                        "名前："+resultItem.getPlayerName(this)+"\n"+
                        "時間："+resultItem.getMovecount()+"\n"+
                        "手数："+resultItem.getPazzleTimeStr(), Toast.LENGTH_SHORT).show();
                pazzle.inactivate();
                pazzle.boardInit();
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnRanking.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(act, RankingActivity.class);
                act.startActivityForResult(intent, RANKING_ACTIVITY);
            }
        });
        ...
        btnStatistics.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(act, StatisticsActivity.class);
                act.startActivityForResult(intent, STATISTICS_ACTIVITY);
            }
        });
